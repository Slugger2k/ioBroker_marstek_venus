...
class Adapter {
	...
	async initStates() {
		...
		await this.setObjectNotExistsAsync("power", {
			type: "channel",
			common: { name: "Power measurements" },
			native: {},
		});
		await this.setObjectNotExistsAsync("energy", {
			type: "channel",
			common: { name: "Energy statistics" },
			native: {},
		});
		await this.setObjectNotExistsAsync("control", {
			type: "channel",
			common: { name: "Control settings" },
			native: {},
		});
		...
		await this.setObjectNotExistsAsync("info.mac", {
			type: "state",
			common: { name: "Device MAC", type: "string", role: "text", read: true, write: false },
			native: {},
		});
		await this.setObjectNotExistsAsync("info.connection", {
			type: "state",
			common: { name: "Connected", type: "boolean", role: "indicator.reachable", read: true, write: false },
			native: {},
		});
		...
		await this.setObjectNotExistsAsync("power.pvVoltage", {
			type: "state",
			common: { name: "PV voltage", type: "number", unit: "V", role: "value", read: true, write: false },
			native: {},
		});
		await this.setObjectNotExistsAsync("power.pvCurrent", {
			type: "state",
			common: { name: "PV current", type: "number", unit: "A", role: "value", read: true, write: false },
			native: {},
		});
		...
		await this.setObjectNotExistsAsync("network", {
			type: "channel",
			common: { name: "Network information" },
			native: {},
		});
		await this.setObjectNotExistsAsync("network.ip", {
			type: "state",
			common: { name: "Device IP", type: "string", role: "text", read: true, write: false },
			native: {},
		});
		...
		await this.setObjectNotExistsAsync("energymeter", {
			type: "channel",
			common: { name: "Energy meter" },
			native: {},
		});
		await this.setObjectNotExistsAsync("energymeter.ctState", {
			type: "state",
			common: {
				name: "CT state (0=disconnected, 1=connected)",
				type: "number",
